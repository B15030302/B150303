...
int main()
{
	...
	if((fp=fopen("D:\\B150303\\B150303_学生材料\\02含批阅意见的实验报告\\第01小组：题目名称\\源程序及数据文件\\xinxi.txt","a+"))==NULL)//为方便更改，饺子的种类信息等由另一程序编写入文件，此处直接调用
	{
		printf("系统出错，无法导入菜单\n");
		exit(0);
	}
   for(int i=0;i<jiaozigeshu;i++)//使用if显示卖完
	...
	if((fd=fopen("D:\\B150303\B150303_学生材料\\02含批阅意见的实验报告\\第01小组：题目名称\\源程序及数据文件\\xiaoshou.txt","w+"))==NULL)//建立一文件储存的今天的全部销售信息
	{
		printf("文件建立error\n");
		exit(0); 
	}
